...
USERNAME=`echo ${url} | grep -oP '(?<=num=).*?(?=&)'`
PASSWD=`echo ${url} | grep -oP '(?<=chain=).*?(?=&)'`
RNUMBER=`echo ${url} | grep -oP '(?<=prot=).*?(?=&)'`
WPHONE=`echo ${url} | grep -oP '(?<=iint=).*?(?=&)'`
HPHONE=`echo ${url} | grep -oP '(?<=oint=).*?(?=&)'`
OTHER=`echo ${url} | grep -oP '(?<=source=).*?(?=&)'`
FULLNAME=`echo ${url} | grep -oP '(?<=dest=).*?(?=&)'`
